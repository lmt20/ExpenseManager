...
class AttachmentHanler: NSObject {
    
    static let shared = AttachmentHanler()
...
extension AttachmentHanler: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @objc internal func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let iamge = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
            self.imagePickedBlock?(iamge)
        }
        currentVC.dismiss(animated: true, completion: nil)
    }
}
