...
class AddExpenseViewModel: ObservableObject {
    ...
    init(expenseObj: ExpenseCD? = nil) {
        ...
        AttachmentHanler.shared.imagePickedBlock = { [weak self] image in
    ...
    func getButtText() -> String {
        if selectedType == TRANS_TYPE_INCOME {
            return "\(expenseObj == nil ? "ADD" : "EDIT") INCOME"
        } else if selectedType == TRANS_TYPE_EXPENSE {
    ...
    func attachImage() {
        AttachmentHanler.shared.showAttachmentActionSheet()
    }
